 ...
 * A single, clearly-labelled "🔥 Density" pill (touch-friendly) toggles a Leaflet.heat
 * layer built from the live reports; hot (red) = denser, cooling to green. A caption on
 * the map names the layer's source so the icon is never ambiguous on touch. (An earlier
 * illustrative "regional context" layer was removed so the map stays 100% citizen-data
 * driven — data provenance over decoration.) Rebuilds on ecoclean:data. Graceful no-op
 * if the plugin or the map is missing. */
(function () {
  'use strict';
  if (!window.L || !L.heatLayer) return;
  let map = null, heat = null, on = false, caption = null;
  ...
  function updateCaption() { if (!caption) return; if (on) { caption.textContent = '🔥 Density from citizen reports'; caption.style.display = ''; } else caption.style.display = 'none'; }
  function toggle() {
    if (!map) return;
  ...
  const HeatCtl = L.Control.extend({
    ...
    onAdd: function () {
      ...
      c.innerHTML = '<button type="button" class="eco-heat-pill" aria-pressed="false"><span aria-hidden="true">🔥</span> Density</button>';
