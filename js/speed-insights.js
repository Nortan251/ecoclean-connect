// Vercel Speed Insights - Inline implementation
// This is a self-contained version that doesn't require bundling
...
function loadProps(explicitProps, confString) {
  ...
  if (confString) {
    try {
      props = {
        ...JSON.parse(confString)?.speedInsights,
        ...explicitProps
      };
    } catch {
    }
...
function injectSpeedInsights(props = {}, confString) {
  ...
  if (beforeSend) {
    window.si?.call(window, "beforeSend", beforeSend);
  }
