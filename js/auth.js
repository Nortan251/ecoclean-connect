 ...
 * Sign-in / account control into the header on every page + an auth modal, and
 * exposes window.EcoAuth so other modules (report submit, the wallet) can read the
 * session / token. It dispatches 'ecoclean:auth' (detail = user|null) on changes. */
(function () {
  'use strict';
  ...
  function inject() {
    ...
    st.textContent =
      '.eco-acct-btn{background:rgba(255,255,255,.18);color:#fff;border:none;border-radius:999px;padding:7px 13px;font-size:.82rem;font-weight:700;cursor:pointer;margin-left:8px;font-family:inherit;}' +
      '.eco-acct-btn:hover{background:rgba(255,255,255,.3);}' +
      '.eco-acct-chip{display:inline-flex;align-items:center;gap:7px;background:rgba(255,255,255,.18);border:none;border-radius:999px;padding:5px 12px 5px 5px;margin-left:8px;cursor:pointer;color:#fff;font-weight:700;font-size:.82rem;font-family:inherit;}' +
  ...
  function renderNav() {
    ...
    if (current) {
      ...
      const b = document.createElement('button'); b.className = 'eco-acct-btn'; b.type = 'button'; b.textContent = 'Sign in';
  ...
  function openAuth() {
    ...
    function draw() {
      modalEl.innerHTML = '<div class="eco-auth-card">' +
        ...
        '<div class="eco-auth-tabs"><button data-m="in" class="' + (mode === 'in' ? 'on' : '') + '">Sign in</button><button data-m="up" class="' + (mode === 'up' ? 'on' : '') + '">Sign up</button></div>' +
        '<form id="ecoAuthForm">' +
        ...
        '<button class="primary-btn" type="submit">' + (mode === 'up' ? 'Create account' : 'Sign in') + '</button>' +
    ...
    function onSubmit(e) {
      ...
      cfgClient().then((c) => {
        ...
        p.then(({ data, error }) => {
          ...
          if (mode === 'up' && data && data.user && !data.session) { msg.className = 'eco-auth-msg ok'; msg.textContent = 'Check your email to confirm, then sign in.'; return; }
  ...
  // --- session ---------------------------------------------------------------
  function refreshMe() {
    fetch('/api/me', { headers: { Authorization: 'Bearer ' + current.accessToken } })
      .then((r) => (r.ok ? r.json() : null)).then((me) => {
        if (me && current) { current.displayName = me.displayName; current.points = me.points; current.vouchers = me.vouchers; current.myReports = me.myReports; }
        renderNav(); emit();
      }).catch(() => { renderNav(); emit(); });
  }
